...
#                is_matching = True
                ...
                if int((batch_size_train+1)/2)+ind_non_match_counter >= np.shape(train_non_match_data):
                    np.random.shuffle(train_non_match_data)
...
#                b_l_val,b_r_val = generator.get_pairs(generator.val_data,val_batch)
                _,train_loss_value,left_o,right_o = sess.run([train_op, train_loss, left_train_output, right_train_output],feed_dict={left_train:b_l_train, right_train:b_r_train, label_train:b_sim_train})
                if i % 100 == 0:
