...
def main(unused_argv):
    ...
    if not os.path.exists(output_dir + ".meta"):
        ...
        tf.add_to_collection("left_output",left_train_output)
        tf.add_to_collection("right_output",right_train_output)
...
#        writer = tf.summary.FileWriter(output_dir + "/summary", graph=tf.get_default_graph())
        ...
        train_non_match_dataset = train_match_dataset.shuffle(buffer_size=np.shape(generator.all_non_match_train)[0])
        train_non_match_dataset = train_match_dataset.repeat()
        train_non_match_dataset = train_match_dataset.batch(batch_size_train)
        ...
        for i in range(1,train_iter + 1):
            if is_matching:
                train_batch = sess.run(next_element,feed_dict={handle:train_match_handle})
                val_batch = sess.run(next_element,feed_dict={handle:val_match_handle})
                b_sim_train = np.ones((batch_size_train,1))
                b_sim_val = np.ones((batch_size_val,1))
                is_matching = False
            else:
                train_batch = sess.run(next_element,feed_dict={handle:train_non_match_handle})
                val_batch = sess.run(next_element,feed_dict={handle:val_non_match_handle})
                b_sim_train = np.zeros((batch_size_train,1))
                b_sim_val = np.zeros((batch_size_val,1))
                is_matching = True
            ...
            _,train_loss_value,val_loss_value,left_o,right_o = sess.run([train_op, train_loss,val_loss, left_train_output, right_train_output],feed_dict={left_train:b_l_train, right_train:b_r_train, label_train:b_sim_train, left_val:b_l_val, right_val:b_r_val, label_val:b_sim_val})
            if i % 10 == 0:
