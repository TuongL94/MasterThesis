...
def evaluate_siamese_network(generator, batch_size, thresholds, eval_itr, output_dir, metrics_path, gpu_device_name, metrics_name, negative_multiplier):
...
#                sim_full = np.vstack((np.ones((batch_size*eval_itr,1)), np.zeros((batch_size*eval_itr,1))))
                ...
                accuracy, false_pos, false_neg, recall, fnr, fpr, inter_class_errors, tnr = get_test_diagnostics(left_full,right_full,sim_full,0.4,class_id, plot_hist = True, breakpoint = breakpoint)
...
def main(argv):
    ...
    """
    # set parameters for evaluation
    thresholds = np.linspace(0, 0.0003, num=100)
    batch_size = 200
    eval_itr = 3
...
#        return
    ...
    with open(data_path + "generator_data_siamese_trans_10.pk1", "rb") as input:
        ...
        evaluate_siamese_network(generator, batch_size, thresholds, eval_itr, output_dir, metrics_path, gpu_device_name, metrics_name, negative_multiplier)
