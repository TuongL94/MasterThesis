# -*- coding: utf-8 -*-
"""
Created on Fri Apr 13 14:34:36 2018
...
#            right_test_inference = tf.get_collection("right_test_output")[0]
        ...
        with tf.Session(config=config) as sess:
            with tf.device(gpu_device_name):
                ...
                test_dataset = tf.data.Dataset.from_tensor_slices(generator.anchors_test)
...
#                current_val_loss = 0
                ...
                for j in range(eval_itr):
                    test_batch_anchors = sess.run(next_element,feed_dict={handle:test_handle})
...
#                            b_anch_val,b_pos_val,b_neg_val= generator.get_triplet(generator.val_data[k], generator.triplets_val, val_batch_anchors, difficulty_lvl)
                        b_anch_test,b_pos_test,b_neg_test = generator.get_triplet(generator.test_data[k], generator.triplets_test, test_batch_anchors)
                        anchor_o,pos_o,neg_o = sess.run([anchor_output, positive_output, negative_output],
                                                                       feed_dict = {anchor_holder:b_anch_test, 
                                                                                    positive_holder:b_pos_test, 
                                                                                    negative_holder:b_neg_test})
...
#                                class_id = np.vstack((class_id, class_id_batch))
                ...
                precision, false_pos, false_neg, recall, fnr, fpr, _ = get_test_diagnostics(anchor_full,candidates_full, labels_full,threshold)
